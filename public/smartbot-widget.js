...
(function () {
  ...
  function push(mount, sender, text) {
    const bubble = el(
      "div",
      sender === "bot" ? "sb-bot" : "sb-user",
      `<b>${sender === "bot" ? "SmartBot" : "Вы"}:</b> ${text}`
    );
    mount.appendChild(bubble);
    mount.scrollTop = mount.scrollHeight;
  }

  // Вызов серверного роутера (ключ хранится на сервере)
  async function callAssistant(messages, job) {
    ...
    return data.reply; // строка-ответ ассистента
  }

  // Глобальная функция для открытия модального виджета
  window.openSmartBotModal = function ({ job, title = "SmartBot", onClose }) {
    ...
    const state = {
      step: 0,
      candidate: {},
      history: [{ role: "system", content: "Ты скрининговый ассистент вакансий." }],
    };

    function addQuick(labels) {
      labels.forEach((l) => {
        const b = el("button", null, l);
        b.onclick = () => sendUser(l);
        quick.appendChild(b);
      });
    }
    ...
    function ask() {
      quick.innerHTML = "";
      if (state.step === 0) {
        push(messages, "bot", `Спасибо за интерес к вакансии «${job.title}». Как вас зовут?`);
      }
      if (state.step === 1) {
        push(messages, "bot", `Вы сейчас в городе ${job.city}?`);
        addQuick(["Да", "Нет"]);
      }
      if (state.step === 2) {
        push(messages, "bot", `Есть минимум ${job.exp}?`);
        addQuick(["Да", "Нет"]);
      }
      if (state.step === 3) {
        push(messages, "bot", `Формат ${job.format}. Подходит?`);
        addQuick(["Да", "Нет"]);
      }
      if (state.step === 4) {
        push(messages, "bot", "Спасибо! Оцениваю вашу релевантность…");
        finish();
      }
    }
    ...
    async function sendUser(text) {
      ...
      if (state.step === 0) {
        state.candidate.name = text;
        state.step = 1;
        ask();
        return;
      }
      if (state.step === 1) {
        state.candidate.city = normalize(text);
        state.step = 2;
        ask();
        return;
      }
      if (state.step === 2) {
        state.candidate.exp = normalize(text);
        state.step = 3;
        ask();
        return;
      }
      if (state.step === 3) {
        state.candidate.format = normalize(text);
        state.step = 4;
        ask();
        return;
      }

      // свободный чат после скоринга
      ...
      try {
        ...
        push(messages, "bot", "Ошибка сервера ассистента: " + (err?.message || "неизвестно"));
      }
    }
    ...
    function finish() {
      ...
      all.push({
        name: state.candidate.name || "Кандидат",
        ...state.candidate,
        score,
        jobId: job.id,
        jobTitle: job.title,
